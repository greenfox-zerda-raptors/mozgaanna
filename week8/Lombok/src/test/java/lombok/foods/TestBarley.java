...
public class TestBarley {
    ...
    @Before
    public void init() {
        barley = Barley.builder().weight(2300L).qualityMultiplier(1.2F).expirationDate(LocalDate.parse("2016-12-15", dateTimeFormatter)).build();
    }

    @Test
    public void testToString() throws Exception {
        String expected = "Barley(super=AbstractHorseFood(super=AbstractFood(name=Barley, weight=2300, calories=11500, creationDate=" + new DateTime().toString("yyyy-MM-dd") + ", qualityMultiplier=1.2), expirationDate=2016-12-15))";
